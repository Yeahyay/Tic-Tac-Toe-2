   ...
   if listeners then
      for i = 1, #listeners do
         ...
         if listener.enabled then
				returnValue = {listener.system[listener.callback](listener.system, ...)}
         end
      end
	else
		error("callback: "..tostring(eventName).." not found")
   end

   return unpack(returnValue) --used to return the instance itself, now it returns everything from the callback
end
